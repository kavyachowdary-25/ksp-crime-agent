...
Respond IMMEDIATELY with the JSON action. Do NOT write analysis, planning,
or reasoning text before the JSON — output the JSON object and stop.
...
2. Database overview — statistics by crime type, district, year, status:
   {"action": "overview", "reason": "..."}
   Use this FIRST for broad questions: "insights", "summary", "what can you
   tell me about the crime data", "overall trends", "give me an overview".
   One call returns all aggregates — never explore with multiple sql_query
   calls for broad questions.

3. Modus operandi patterns — signature terms per crime type from FIR facts:
   {"action": "mo_patterns", "reason": "..."}
   Use for questions about modus operandi, MO, "how are crimes committed",
   or comparing crime patterns/methods across types or the whole database.

4. Resolved person criminal history (handles name spelling variants):
   {"action": "person_history", "name": "Manjunath", "reason": "..."}

5. Offender risk score (transparent, factor-by-factor):
   {"action": "risk_score", "name": "Manjunath", "reason": "..."}
   Use for "how dangerous/risky is X", "risk assessment of X", "should we
   prioritize X". Present the factor contributions, never just the number.

6. Similar past cases to a given FIR (narrative similarity + outcomes):
   {"action": "similar_cases", "crimeno": "100010007202100002", "reason": "..."}
   Use for "cases similar to FIR/CrimeNo X", "have we seen this pattern before".

7. Connection path between two people (degrees of separation via shared cases):
   {"action": "connection_path", "from": "Manjunath", "to": "Syed Imran", "reason": "..."}
   Use for "how is X connected/linked/related to Y" questions.

8. Co-accused network of a person:
   {"action": "co_accused", "name": "Syed Imran", "reason": "..."}

9. Ask the user a clarifying question (ONLY when the request is truly
   ambiguous and no tool fits):
...
- When answering in Kannada, keep the answer under 60 words and list at most
  5 items — Kannada text is longer; be extra brief.
...
- For "timeline of X" requests, use person_history and present the cases in
  chronological order with dates.
- Vague requests like "find patterns", "any insights", "tell me something
  interesting" -> do NOT ask for clarification; call mo_patterns or overview
  and present what the data shows.
...
def run_agent(user_message: str, history: list, llm, backend) -> AgentTurn:
    ...
    for step in range(MAX_STEPS):
        ...
        if kind == "clarify":
            turn.answer = action.get("question", "Could you clarify what you're looking for?")
            turn.citations = []
            return turn
        ...
        if kind == "sql_query":
            ...
            obs = backend.overview()
        elif kind == "mo_patterns":
            obs = backend.mo_patterns()
        elif kind == "person_history":
            obs = backend.person_history(action.get("name", ""))
        elif kind == "risk_score":
            obs = backend.risk_score(action.get("name", ""))
        elif kind == "similar_cases":
            obs = backend.similar_cases(action.get("crimeno", ""))
        elif kind == "connection_path":
            obs = backend.connection_path(action.get("from", ""), action.get("to", ""))
        ...
        messages.append({"role": "user", "content":
            ...
            "final_answer must be single-line JSON, answer under 100 words."})
